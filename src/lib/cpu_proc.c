...
static void proc_ld(cpu_context* ctx) {
    if (ctx->dest_is_mem) {
        ...
        if (is_16_bit(ctx->curr_inst->reg_2)) {
            //if 16 bit register...
            emu_cycles(1);
...
static void proc_inc(cpu_context* ctx) {
    u16 val = cpu_read_reg(ctx->curr_inst->reg_1) + 1;

    if (is_16_bit(ctx->curr_inst->reg_1)) {
        emu_cycles(1);
    }

    if (ctx->curr_inst->reg_1 == RT_HL && ctx->curr_inst->mode == AM_MR) {
        val = bus_read(cpu_read_reg(RT_HL)) + 1;
        val &= 0xFF;
        bus_write(cpu_read_reg(RT_HL), val);
    } else {
        cpu_set_reg(ctx->curr_inst->reg_1, val);
        val = cpu_read_reg(ctx->curr_inst->reg_1);
    }
...
static void proc_sub(cpu_context* ctx) {
    u16 val = cpu_read_reg(ctx->curr_inst->reg_1) - ctx->fetched_data;

    int z = val == 0;
    int h = ((int)cpu_read_reg(ctx->curr_inst->reg_1) & 0xF) - ((int)ctx->fetched_data & 0xF) < 0;
    int c = ((int)cpu_read_reg(ctx->curr_inst->reg_1)) - ((int)ctx->fetched_data) < 0;
...
static void proc_sbc(cpu_context* ctx) {
    ...
    int h = ((int)cpu_read_reg(ctx->curr_inst->reg_1) & 0xF) 
        - ((int)ctx->fetched_data & 0xF) - ((int)CPU_FLAG_C) < 0;
    int c = ((int)cpu_read_reg(ctx->curr_inst->reg_1)) 
...
static void proc_add(cpu_context* ctx) {
    u32 val = cpu_read_reg(ctx->curr_inst->reg_1) + ctx->fetched_data;
    ...
    int z = (val & 0xFF) == 0;
    int h = (cpu_read_reg(ctx->curr_inst->reg_1) & 0xF) + (ctx->fetched_data & 0xF) >= 0x10;
    int c = (int)(cpu_read_reg(ctx->curr_inst->reg_1) & 0xFF) + (int)(ctx->fetched_data & 0xFF) >= 0x100;

    if (is_16bit) {
        z = -1;
        h = (cpu_read_reg(ctx->curr_inst->reg_1) & 0xFFF) + (ctx->fetched_data & 0xFFF) >= 0x1000;
        u32 n = ((u32)cpu_read_reg(ctx->curr_inst->reg_1)) + ((u32)ctx->fetched_data);
        c = n >= 0x10000;
    }
